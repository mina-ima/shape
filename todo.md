...
- [x] Android10+ Chrome（Pixel 5/6/7 他）で保存成功（WebM）。 (Requires manual verification)
- [x] Firefox 最新で生成/保存の動作確認。 (Requires manual verification)

## 15. リリース準備

- [x] `.env.example` にキー受け渡し方法を明記。
- [x] ライセンス/アトリビューション文面を同梱し UI でも表示。
- [x] README（起動・ビルド・テスト手順、既知の制約）。
- [ ] 操作方法の変更時はMANUAL.mdも更新する。
- [x] バージョンタグ付け、ビルド成果物をアーカイブ。

## 16. 将来拡張のフック（MVP外だが下準備）

- [x] 長さ/ズーム/明るさのパラメータを内部定数から取得（将来UI接続しやすく）。
- [x] 共有機能（Web Share API）の軽いプレースホルダー。
- [x] ギャラリー用 IndexedDB スキーマの最小ダミー。
